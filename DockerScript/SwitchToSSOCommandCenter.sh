#!/bin/bash
#
# Update an existing Command Center (non-SSO) to Command Center with SSO
#
# Author: Lili Ji
...
echo
echo "Updating the non-SSO Command Center Server to the SSO version..."
echo
...
sudo docker exec --user root $cc_container_id java -jar /usr/local/tomcat/lib/Lingoport_Resource_Manager_Server-14.1-Installer.jar /usr/local/tomcat/auto-update.xml
sudo docker exec --user root $cc_container_id chown -R tomcatuser:tomcatgroup /usr/local/tomcat/lingoport/lrm-server-14.1
