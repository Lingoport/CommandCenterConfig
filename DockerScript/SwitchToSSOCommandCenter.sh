...
sudo docker exec --user root $cc_container_id java -jar /usr/local/tomcat/lib/Lingoport_Resource_Manager_Server-14.0-Installer.jar /usr/local/tomcat/auto-update.xml
sudo docker exec --user root $cc_container_id chown -R tomcatuser:tomcatgroup /usr/local/tomcat/lingoport/lrm-server-14.0
...
sudo docker exec $cc_container_id bash -c '[ -f /usr/local/tomcat/lingoport/lrm-server-14.0/lib/lc-public-api-sdk-24.0.5Lingoport.jar ] && cp /usr/local/tomcat/lingoport/lrm-server-14.0/lib/lc-public-api-sdk-24.0.5Lingoport.jar /usr/local/tomcat/webapps/command-center/WEB-INF/lib'
sudo docker exec $cc_container_id sh -c "cp -r /usr/local/tomcat/commandcenter/config/* /usr/local/tomcat/lingoport/lrm-server-14.0/lib"
sudo docker exec --user root $cc_container_id rm -f /usr/local/tomcat/lingoport/lrm-server-14.0/lib/aws-java-sdk-1.12.496.jar
